...
const (
	announcePattern            = `(.*?) - (.*) \[([\d]{4})\] \[(Album|Soundtrack|Compilation|Anthology|EP|Single|Live album|Remix|Bootleg|Interview|Mixtape|Demo|Concert Recording|DJ Mix|Unknown)\] - (FLAC|MP3|AAC|DSD) \/ (Lossless|24bit Lossless|V0 \(VBR\)|V2 \(VBR\)|320|256|DSD64|DSD128|DSD256|DSD512) \/( (Log) \/)?( (-*\d+)\% \/)?( (Cue) \/)? (CD|DVD|Vinyl|Soundboard|SACD|DAT|Cassette|WEB|Blu-Ray) (\/ (Scene) )?(?:\/ (Freeleech! ))?- (http[s]?:\/\/[\w\.\/:]*torrents\.php\?id=[\d]*) \/ (http[s]?:\/\/[\w\.\/:]*torrents\.php\?action=download&id=[\d]*) - ([\w\., ]*)`
	alternativeAnnouncePattern = `(.*?) - (.*) \[([\d]{4})\] \[(Album|Soundtrack|Compilation|Anthology|EP|Single|Live album|Remix|Bootleg|Interview|Mixtape|Demo|Concert Recording|DJ Mix|Unknown)\] - (FLAC|MP3|AAC|DSD) / (Lossless|24bit Lossless|V0 \(VBR\)|V2 \(VBR\)|320|256|DSD64|DSD128|DSD256|DSD512) /( (Log.*?) /)?( (-*\d+)\% /)?( (Cue) /)? (CD|DVD|Vinyl|Soundboard|SACD|DAT|Cassette|WEB|Blu-Ray) (/ (Scene) )?- ([\w\., ]*) - (http[s]?://[\w\./:]*torrents\.php\?id=[\d]*) / (http[s]?://[\w\./:]*torrents\.php\?action=download&id=[\d]*)`
)
